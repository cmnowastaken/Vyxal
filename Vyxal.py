...
def deref(lhs, listify=True):
    if VY_type(lhs) is LazyList:
        return [lhs.safe, lhs.listify][listify]()
    if type(lhs) not in [int, float, str]:
        return list(map(deref, lhs))
    return lhs
...
def negate(lhs):
    return {number: lambda: -lhs, str: lambda: lhs.swapcase()}.get(
        VY_type(lhs), lambda: vectorise(negate, lhs)
    )()
...
def realify(lhs):
    if isinstance(lhs, sympy.core.numbers.ComplexInfinity) or isinstance(
        lhs, sympy.core.numbers.NaN
    ):
        return 0
    else:
        return lhs
...
def replace(haystack, needle, replacement):
    ...
    if t_haystack is list:
        ...
        return replace(
            haystack.listify(), needle, replacement
...
def vectorise(fn, left, right=None, third=None, explicit=False):
    if third:
        ...
        ret = {
            (ts[0], ts[1]): (
                lambda: apply(fn, left, right),
                lambda: expl(iterable(left), right),
            ),
            (list, ts[1]): (
                lambda: [apply(fn, x, right) for x in left],
                lambda: expl(left, right),
            ),
            (ts[0], list): (
                lambda: [apply(fn, left, x) for x in right],
                lambda: swapped_expl(left, right),
            ),
            (LazyList, ts[1]): (lambda: expl(left, right), lambda: expl(left, right)),
            (ts[0], LazyList): (
                lambda: swapped_expl(left, right),
                lambda: swapped_expl(left, right),
            ),
            (list, list): (lambda: gen(), lambda: expl(left, right)),
            (LazyList, LazyList): (lambda: gen(), lambda: expl(left, right)),
            (list, LazyList): (lambda: gen(), lambda: expl(left, right)),
            (LazyList, list): (lambda: gen(), lambda: expl(left, right)),
        }[ts][explicit]()
...
def VY_eval(lhs):
    if online_version or safe_mode:
        try:
            return pwn.safeeval.const(lhs)
        except:
            t = parse(lhs)
            if len(t) and t[-1][0] in (
                Structure.NUMBER,
                Structure.STRING,
                Structure.LIST,
            ):
...
def VY_filter(lhs, rhs):
    ...
    return {
        ...
        (types.FunctionType, types[1]): lambda: LazyList(
            _filter(lhs, iterable(rhs, range))
        ),
        (types[0], types.FunctionType): lambda: LazyList(
            _filter(rhs, iterable(lhs, range))
        ),
    }[ts]()
...
def VY_reduce(lhs, rhs):
    ...
    if isinstance(rhs, types.FunctionType):
        ...
        function, vector = lhs, iterable(rhs, range)
    if not isinstance(vector, LazyList):
        vector = iter(vector)
...
"""
                        ...
                        compiled += tab(
                            f"parameters += pop(parameter_stack, {parameter})"
                        )
                    else:
            ...
            compiled += (
                tab(
                    f"if arity != {defined_arity} and arity >= 0: parameters = pop(parameter_stack, arity); stack = parameters[::]"
                )
                + newline
            )
            compiled += (
                tab(
                    "elif stored: parameters = pop(parameter_stack, stored); print(parameters); stack = parameters[::]"
                )
                + newline
            )
            if defined_arity == 1:
            ...
            if token_value[0] in Grouping_Transformers:
                compiled += (
                    transpile([(Structure.LAMBDA, {Keys.LAMBDA_BODY: token_value[1]})])
                    + newline
                )
            else:
                function_A = transpile(wrap_in_lambda(token_value[1][0]))
            ...
            compiled += (
                transpile([(Structure.LAMBDA, {Keys.LAMBDA_BODY: token_value})])
                + newline
            )
